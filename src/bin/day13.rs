...
fn main() {
	...
	let mut points = lines.lines().map(Point::parse).collect::<HashSet<Point>>();
	let lines = fs::read_to_string("inputs/13-2").unwrap();
	let folds = lines.lines().map(Fold::parse).collect::<Vec<Fold>>();
