...
fn main() {
	let lines = fs::read_to_string("inputs/14-1").unwrap();
	let polymer = lines.lines().next().unwrap().chars().collect::<Vec<char>>();
	let lines = fs::read_to_string("inputs/14-2").unwrap();
	let rules = lines
		.lines()
		.map(|l| {
